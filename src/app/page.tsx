...
export default async function Home(): Promise<JSX.Element> {
  ...
  const components: Components = {
    a: ({ href, children, ...props }) => {
      if (href?.startsWith('/book-a-consultation')) {
        return (
          <Link 
            href={href} 
            className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 no-underline shadow-md hover:shadow-lg mr-4 mb-2"
          >
            {children}
          </Link>
        );
      ...
      if (href?.startsWith('/case-study')) {
        return (
          <Link 
            href={href} 
            className="inline-block bg-gray-600 hover:bg-gray-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 no-underline shadow-md hover:shadow-lg mr-4 mb-2"
          >
            {children}
          </Link>
        );
    ...
    h1: ({ children }) => (
      <h1 className="text-5xl font-bold text-gray-900 mb-6 leading-tight text-center">{children}</h1>
    ),
    h2: ({ children }) => (
